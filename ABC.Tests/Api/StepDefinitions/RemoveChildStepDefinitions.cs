...
public class RemoveChildStepDefinitions
{
    ...
    public RemoveChildStepDefinitions(ApiStartupFixture fixture)
    {
    ...
    {
        A.CallTo(() => _uowFake.Children.GetAsync(
            A<Expression<Func<Child, bool>>>.Ignored, A<CancellationToken>.Ignored))
        .Returns(new[]
        {
    ...
    public void GivenTheChildWithThatIdDoesNotExistInTheDatabase()
    {
        A.CallTo(() => _uowFake.Children.GetAsync(
            A<Expression<Func<Child, bool>>>.Ignored, A<CancellationToken>.Ignored))
        .Returns(Array.Empty<Child>().AsQueryable());
