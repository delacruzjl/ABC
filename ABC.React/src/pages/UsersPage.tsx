...
export const UsersPage: React.FC = () => {
  ...
  const [deactivateUser] = useMutation(DEACTIVATE_USER_MUTATION, {
    onCompleted: () => { refetch(); setActionError(null) },
  ...
  const [deleteUser] = useMutation(DELETE_USER_MUTATION, {
    onCompleted: () => { refetch(); setActionError(null); setConfirmDelete(null) },
    onError: (err) => { setActionError(err.message); setConfirmDelete(null) },
  })
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-cyan-400 mb-6">User Management</h1>
        ...
        <table className="w-full text-left">
          <thead>
            <tr className="border-b border-slate-700">
              <th className="py-3 px-4 text-slate-400 text-sm font-medium">Email</th>
              <th className="py-3 px-4 text-slate-400 text-sm font-medium">Role</th>
              <th className="py-3 px-4 text-slate-400 text-sm font-medium">Status</th>
              <th className="py-3 px-4 text-slate-400 text-sm font-medium">Data</th>
              <th className="py-3 px-4 text-slate-400 text-sm font-medium">Actions</th>
            </tr>
          </thead>
          <tbody>
            {users.map((u) => {
              ...
              return (
                <tr key={u.id} className="border-b border-slate-700/50 hover:bg-slate-700/30">
                  ...
                  <td className="py-3 px-4 text-slate-400 text-sm">
                    {u.hasChildren && (
                      <span className="mr-2" title="Has children">
                        👶 Children
                      </span>
                    )}
                    {u.hasObservations && (
                      <span title="Has observations">📋 Observations</span>
                    )}
